    ...
    describe('ODataResources Service', function() {
        ...
        describe('Resource with param', function() {
            ...
            it('filters should work with funcs', function() {
                ...
                User.odata().filter(new $odata.Func("endswith", "Name", "Raphael"), true).query();
                $httpBackend.flush();
                expect(1).toBe(1);
            });
            ...
            it('should work with complex queries', function() {
                ...
                User.odata().filter("Name", "Raphael").filter("Age", ">", 20).skip(10).take(20).orderBy("Name", "desc").query();
                $httpBackend.flush();
                expect(1).toBe(1);
            });
            ...
            it('should call the callback on success', function() {
                ...
                User.odata().query(success, error);
                $httpBackend.flush();
            ...
            it('should query with expand and filters', function() {
                ...
                User.odata().filter("City", "France").expand("Orders").query();
                $httpBackend.flush();
                expect(1).toBe(1);
            });
            describe('Single method', function() {
                ...
                it('should execute the query but return a single element', function() {
                    $httpBackend.expectGET("/user?$filter=City eq 'France'&$expand=Orders").respond(200, [1, 2]);
                    var result = User.odata().filter("City", "France").expand("Orders").single();
                    $httpBackend.flush();
        ...
        describe('get method', function() {
            ...
            it('should call the callbacks on success', function() {
                ...
                var error = jasmine.createSpy('error');
                $httpBackend.expectGET("/user(2)").respond({});
                User.odata().get(2, success, error);
                $httpBackend.flush();
                expect(success).toHaveBeenCalled();
                expect(error).not.toHaveBeenCalled();
            });
        ...
        describe('OData v4 explicitly specified', function() {
            ...
            describe('should do the expand in a odata v4 way', function() {
                ...
                it('with query and 1 nested element', function() {
                    ...
                    $httpBackend.expectGET("/user?$expand=roles($expand=role)").respond(200, {
                        "@odata.context": "http://host/service/$metadata#Collection(Edm.String)",
                        "value": [{
                            name: 'Test',
                            id: 1,
                ...
                it('with query and 2 nested element', function() {
                    ...
                    $httpBackend.expectGET("/user?$expand=roles($expand=role($expand=users))").respond(200, {
                        "@odata.context": "http://host/service/$metadata#Collection(Edm.String)",
                        "value": [{
                            name: 'Test',
                            id: 1,
                    ...
                    var result = User.odata().expand("roles", "role","users").query();
                    $httpBackend.flush();
                    expect(1).toBe(1);
                });
                ...
                it('with get and 1 nested element', function() {
                    ...
                    $httpBackend.expectGET("/user(1)?$expand=roles($expand=role)").respond(200, {
                        "@odata.context": "http://host/service/$metadata#Collection(Edm.String)",
                        "value": {
                            name: 'Test',
                            id: 1,
                    ...
                    var result = User.odata().expand("roles", "role").get(1);
                    $httpBackend.flush();
                    expect(1).toBe(1);
                });
                ...
                it('with get and 2 nested element', function() {
                    ...
                    $httpBackend.expectGET("/user(1)?$expand=roles($expand=role($expand=users))").respond(200, {
                        "@odata.context": "http://host/service/$metadata#Collection(Edm.String)",
                        "value": {
                            name: 'Test',
                            id: 1,
                    ...
                    var result = User.odata().expand("roles", "role","users").get(1);
                    $httpBackend.flush();
                    expect(1).toBe(1);
                });
                ...
                it('should query with nested odata v4 datetime', function() {
                    ...
                    $httpBackend.expectGET("/user(1)?$filter=(date eq 2015-07-28T10:23:00Z) and (date eq 2016-07-28T10:23:00Z)").respond(200, {
                        "@odata.context": "http://host/service/$metadata#Collection(Edm.String)",
                        "value": {
                            name: 'Test',
                            id: 1,
